...
def web_search_news(query: str, max_results: int = 4) -> Union[Dict[str, Any], ToolErrorResponse]:
    ...
    """
    from src.common.logger import log_tool_intent, log_tool_outcome
    log_tool_intent(agent_name="searcher_agent", tool_name="web_search_news", task_id="task_search", session_id="active_session", inputs={"query": query, "max_results": max_results})
    try:
        if not query or not query.strip():
            ...
            log_tool_outcome(agent_name="searcher_agent", tool_name="web_search_news", task_id="task_search", session_id="active_session", status="ERROR", result=err)
            return err
        ...
        bundle = SearchBundleSchema(query=query, results_count=len(articles), articles=articles).model_dump()
        log_tool_outcome(agent_name="searcher_agent", tool_name="web_search_news", task_id="task_search", session_id="active_session", status="SUCCESS", result=bundle)
        return bundle
